...
class WhatsApp:
    ...
    def __init__(self, wait, screenshot=None, session=None):
        ...
        if session:
            ...
            self.browser = webdriver.Chrome(
                options=chrome_options, executable_path=conf.CHROME_DRIVER_PATH)
        else:
            self.browser = webdriver.Chrome(
    ...
    def send_picture(self, name, picture_location, caption=None):
        ...
        try:
            ...
            time.sleep(1)
            attach_img_btn = self.browser.find_element_by_xpath(
                attach_type_xpath)
            ...
            if caption:
                ...
                send_caption = self.browser.find_element_by_xpath(
                    caption_xpath)
    ...
    def clear_chat(self, name):
        self.browser.find_element_by_css_selector(
            "._3FRCZ").send_keys(name+Keys.ENTER)
    ...
    def set_group_picture(self, group_name, picture_location):
        ...
        try:
            ...
            time.sleep(1)
            exit_group_info = self.browser.find_element_by_xpath(
                exit_group_info_xpath)
    ...
    def get_invite_link_for_group(self, groupname):
        ...
        self.browser.find_element_by_css_selector(
            "#main > header > div._5SiUq > div._16vzP > div > span").click()
        try:
            # time.sleep(3)
            WebDriverWait(self.browser, self.timeout).until(EC.presence_of_element_located(
    ...
    def exit_group(self, group_name):
        ...
        search.send_keys(group_name+Keys.ENTER)
        self.browser.find_element_by_css_selector(
            "._2zCDG > span:nth-child(1)").click()
        WebDriverWait(self.browser, self.timeout).until(EC.presence_of_element_located(
            (By.CSS_SELECTOR, "div._1CRb5:nth-child(6) > div:nth-child(1) > div:nth-child(2) > div:nth-child(1) > span:nth-child(1)")))
        time.sleep(3)
    ...
    def send_anon_message(self, phone, text):
        payload = urlencode(
            {"phone": phone, "text": text, "source": "", "data": ""})
        ...
        send_message = self.browser.find_element_by_css_selector(
            "#action-button")
        send_message.click()
    ...
    def is_message_present(self, username, message):
        ...
        try:
            WebDriverWait(self.browser, self.timeout).until(EC.presence_of_element_located(
                (By.XPATH, "/html/body/div[1]/div/div/div[2]/div[3]/span/div/div/div[2]/div/div/div/div/div[1]/div/div/div/div[2]/div[1]/span/span/span")))
            return True
        except TimeoutException:
            return False
    ...
    def get_starred_messages(self, delay=10):
        ...
        for message in messages:
            try:
                ...
                to = soup.find(
                    "div", class_="copyable-text")["data-pre-plain-text"]
                message_text = soup.find(
                    "span", class_="selectable-text invisible-space copyable-text").text
                message.click()
                ...
                WebDriverWait(self.browser, 5).until(EC.presence_of_element_located(
                    (By.CSS_SELECTOR, "div._14oqx:nth-child(3) > div:nth-child(1) > div:nth-child(1) > span:nth-child(1) > span:nth-child(1)")))
